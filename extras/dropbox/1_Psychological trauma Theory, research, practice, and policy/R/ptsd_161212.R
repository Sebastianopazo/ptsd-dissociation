#########en qu?? estoy: quiero rehacer los an??lisis para ver que efectivamente con un 
  #subset de los datos las regresiones que mostre a Rodrigo siguen siendo signficativas. 
  #Si lo son, entender porqu?? los modelos de mediacion no lo son. Luego decidir si probar
  #otros modelos de mediacion. Hacer esto rapido para definir los resultados a usar y crear
  #outline de paper para mandar muy luego a Rodrigo
###### NOTES ####
  
# This syntax was created by Paula Err?zuriz starting on Sept 2016 to work with the data of 
...
# Select code book variables & data from patients who all t1 data
# Skip this step to use previous data selection
mydata <- subset(mydata, BDI_t1 != is.na(1) & PCL_t1 != is.na(1) 
                 & CIDI_K_t1 != is.na(1), 
                 select=c("Nreclutado","Sexo", "Edad", "CIDI_A8_basal",
                          "tipo_trauma", "PDEQ_t0", "PDI_t0", "TQ_t0",
...
##### Correlations #### !!!!!QUIERO CORRELACIONAR TODAS LAS VARIABLES Y NO ME RESULTA

hola <- corr.p(mydata[1:65, ],mydata,adjust="holm",alpha=.05)
hola <- mydata[c(-2,-5,-14,-15)]

hola2 <- cor(x = hola, use = "na.or.complete")
hola3 <- round(hola2, digits = 2)
