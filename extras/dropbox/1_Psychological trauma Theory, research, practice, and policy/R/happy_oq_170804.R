...
# Keep only one observation for each client
# Create way to identify first session of a client
# Only keep information for first session of client to avoid duplicating data because it used to be wide
# Codificar valores de OQ dif y feliz dif que los hagan mas faciles de interpretar
# Aprender a ver trayectorias (graficar trayectorias de Felicidad, OQ total y sintomas OQ)

###### NOTES ####

###OJO: ACTUALIZAR PARA USAR CON LA ULTIMA BASE DE DATOS (CON OQ DIF ARREGLADO)
...
#oqdif; difference between score of first and last oq of each client (session 1 - last session)
#feliz; life satisfaction (happiness score ??? question from work value survey)
#felizdif; change in happiness during treatment; first feliz - last feliz
#vfelizdif; level of change in happines: group 1 (decrease), 2 (no change), 3 (increase)
#iat; wai score. Can go from 12 to 84 score.  Greater score, better alliance.
#iatdif; change in wai during treatment
#ep*; ep*; demographic information
...
# Create and recode vfelizdif (value of difference in feliz)
mydata$vfelizdif <- mydata$felizdif
attach(mydata)
mydata$vfelizdif[vfelizdif > 0] <- 1 # Happiness decreased
mydata$vfelizdif[vfelizdif == 0] <- 2 # Happiness didn??t change
mydata$vfelizdif[vfelizdif < 0] <- 3 # Happiness increased
detach(mydata)
...
wide6 <- reshape(mydata, v.names = "oq1", "oq2", "oq3", "oq4", "oq5", "oq6", "oq7", "oq8", "oq9", "oq10", "oq11",
                 "oq12", "oq13", "oq14", "oq15", "oq16", "oq17", "oq18", "oq19", "oq20", "oq21", "oq22", "oq23", "oq24",
                 "oq25", "oq26", "oq27", "oq28", "oq29", "oq30", "oq","feliz", "iat", 
                 varing = "grupo", "iatdif", 
                 idvar = "pacien", timevar = "sesion", direction = "wide")
...
## Keep only clients that have data for more than one session. 
mydata <- mydata[ which(mydata$nsesion > 1), ]

## Select data depending on changes in happiness 

# Happiness decreased
mydata1 <- mydata[ which(mydata$felizdif > 0), ]

# Happiness didn??t change
mydata2 <- mydata[ which(mydata$felizdif == 0), ]

# Happiness increased
mydata3 <- mydata[ which(mydata$felizdif < 0), ]
...
plot(mydata3$oqdif,mydata3$felizdif)
...
# Diferences in changes in OQ according to level of change in happiness
fit <- aov(oqdif ~ vfelizdif, data=mydata, projections = FALSE, qr = TRUE,
           contrasts = NULL)
fit
summary(fit)
coefficients(fit)
...
#change data bases (add number after mydata) to change analyses

# Happiness with oq total
r3 <- lm(formula = feliz ~ oq + age + male0 + aedu + iat, data = mydata3)
r3
summary(r3)

# Happiness with changes in oq total
r4 <- lm(formula = felizdif ~ oqdif + age + male0 + aedu + iat, data = mydata3)
r4
summary(r4)
#Mientras menos aumenta la feliciad, mas disminuye el malestar psicologico

# Happiness with oq total and oq dif
r3 <- lm(formula = feliz ~ oq + oqdif + age + male0 + aedu + iat, data = mydata3)
r3
summary(r3)

# Happiness with oq items
r3 <- lm(formula = feliz ~ oq1 + oq2 + oq3 + oq4 + oq5 + oq6 + oq7 + oq8 + oq9 + oq10 + oq11 + oq12 + oq13 + oq14 + oq15 + oq16 
         + oq17 + oq18 + oq19 + oq20 + oq21 + oq22 + oq23 + oq24 + oq25 + oq26 + oq27 + oq28 + oq29 + oq30 + age + male0 + aedu 
         + iat, data = mydata)
r3
summary(r3)

# Changes in happiness with oq items
r4 <- lm(formula = felizdif ~ oq1 + oq2 + oq3 + oq4 + oq5 + oq6 + oq7 + oq8 + oq9 + oq10 + oq11 + oq12 + oq13 + oq14 + oq15 + oq16 
         + oq17 + oq18 + oq19 + oq20 + oq21 + oq22 + oq23 + oq24 + oq25 + oq26 + oq27 + oq28 + oq29 + oq30 + age + male0 + aedu 
         + iat, data = mydata3)
r4
summary(r4)
