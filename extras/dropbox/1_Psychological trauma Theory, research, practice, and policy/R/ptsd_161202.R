...
###### PREPARING DATA (run always) ####

# Select code book variables & data from patients who have t0 and t1 data
# !!!!Skip this step to use full data base
mydata <- subset(mydata, BDI_t1 != is.na(1) | PCL_t1 != is.na(1) 
                 | CIDI_K_t1 != is.na(1), 
...
library(mediation)
med.fit<-lm(data_m1$PDEQ_t0~data_m1$TQ_t0)   #med.fit<-lm(M ~X) - Ecuación de predicción del mediador (M) por la variables independiente (X)
summary(med.fit)
out.fit<-glm(data_m1$PCL_t1~data_m1$TQ_t0+data_m1$PDEQ_t0) #out.fit<-glm(D~ X+M) - Ecuación de predicción de la variable dependiente (D) por la variables independiente (X) y mediadora
summary(out.fit)
med.out<-mediate(med.fit,out.fit, treat="data_m1$TQ_t0", sims=10000, mediator="data_m1$PDEQ_t0", robustSE=TRUE,  dropobs = TRUE) #Para obtener los coeficientes de medicación con con significación estimada con 10000 bootstraps
summary(med.out) 
...
library(mediation)
med.fit<-lm(data_m2$PDEQ_t0~data_m2$TQ_t0)   #med.fit<-lm(M ~X) - Ecuación de predicción del mediador (M) por la variables independiente (X)
summary(med.fit)
out.fit<-glm(data_m2$BDI_t1~data_m2$TQ_t0+data_m2$PDEQ_t0) #out.fit<-glm(D~ X+M) - Ecuación de predicción de la variable dependiente (D) por la variables independiente (X) y mediadora
summary(out.fit)
med.out<-mediate(med.fit,out.fit, treat="data_m2$TQ_t0", sims=10000, mediator="data_m2$PDEQ_t0", robustSE=TRUE,  dropobs = TRUE) #Para obtener los coeficientes de medicación con con significación estimada con 10000 bootstraps
summary(med.out) 
...
library(mediation)
med.fit<-lm(data_m3$PDI_t0~data_m3$TQ_t0)   #med.fit<-lm(M ~X) - Ecuación de predicción del mediador (M) por la variables independiente (X)
summary(med.fit)
out.fit<-glm(data_m3$PCL_t1~data_m3$TQ_t0+data_m3$PDI_t0) #out.fit<-glm(D~ X+M) - Ecuación de predicción de la variable dependiente (D) por la variables independiente (X) y mediadora
summary(out.fit)
med.out<-mediate(med.fit,out.fit, treat="data_m3$TQ_t0", sims=10000, mediator="data_m3$PDI_t0", robustSE=TRUE,  dropobs = TRUE) #Para obtener los coeficientes de medicación con con significación estimada con 10000 bootstraps
summary(med.out) 
...
library(mediation)
med.fit<-lm(data_m4$PDI_t0~data_m4$TQ_t0)   #med.fit<-lm(M ~X) - Ecuación de predicción del mediador (M) por la variables independiente (X)
summary(med.fit)
out.fit<-glm(data_m4$BDI_t1~data_m4$TQ_t0+data_m4$PDI_t0) #out.fit<-glm(D~ X+M) - Ecuación de predicción de la variable dependiente (D) por la variables independiente (X) y mediadora
summary(out.fit)
med.out<-mediate(med.fit,out.fit, treat="data_m4$TQ_t0", sims=10000, mediator="data_m4$PDI_t0", robustSE=TRUE,  dropobs = TRUE) #Para obtener los coeficientes de medicación con con significación estimada con 10000 bootstraps
summary(med.out) 
