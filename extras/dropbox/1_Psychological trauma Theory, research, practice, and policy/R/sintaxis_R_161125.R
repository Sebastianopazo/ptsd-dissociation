...
# Install and Call packages installed to work on this project (not sure if all are needed)
...
Te recomiendo usar el programa "mediation" (documento adjunto). Los comandos básicos son:
  
install.packages("mediation") #Sólo la primera vez que se instala el paquete

library (mediation) #Cada vez que se inicia una sesión para cargar el paquete

med.fit<-lm(M ~X)    #Ecuación de predicción del mediador (M) por la variables independiente (X)

out.fit<-glm(D~ X+M) #Ecuación de predicción de la variable dependiente (D) por la variables independiente (X) y mediadora
